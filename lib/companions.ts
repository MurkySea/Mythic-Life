import {
  ...
  relationshipStage,
  type CompanionDef,
} from './companions-base'

/**
 * Canonical Mythic Life roster.
 *
 * `seraphine` remains the founding slot's internal key because older chat,
 * memory, party and notification data already use it. User-facing, that slot
 * is Elowen. The actual Seraphine is a distinct companion with her own stable
 * slug, `seraphine_quietflame`.
 *
 * Keep this file as the single roster entrypoint. Older imports of
 * `@/lib/companions` therefore receive the same identity model as newer code.
 */
...
export type { CompanionDef } from './companions-base'
